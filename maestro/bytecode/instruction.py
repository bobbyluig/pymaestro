from maestro.bytecode.protocol import Opcode
...
class BytecodeInstruction:
    ...
    def toByteList(self):
        ...
        if self.opcode == Opcode.LITERAL or self.opcode == Opcode.JUMP or (
                        self.opcode == Opcode.JUMP_Z or self.opcode == Opcode.CALL):
    ...
    def newJumpToLabel(name, filename, column_number, line_number):
        return BytecodeInstruction(Opcode.JUMP, filename, column_number, line_number, isJumpToLabel=True,
                                   labelName=name)

    @staticmethod
    def newConditionalJumpToLabel(name, filename, column_number, line_number):
        return BytecodeInstruction(Opcode.JUMP_Z, filename, column_number, line_number, isJumpToLabel=True,
                                   labelName=name)
